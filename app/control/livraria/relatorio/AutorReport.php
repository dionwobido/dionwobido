 ...
 */
class AutorReport extends TPage
{
    protected $form; // form
    ...
    function __construct()
    {
        ...
        // creates the form
        $this->form = new BootstrapFormBuilder('form_Autor_report');
        $this->form->setFormTitle('Autor Report');
    ...
    function onGenerate()
    {
        try
        {
            ...
            $repository = new TRepository('Autor');
            ...
            if ($data->email)
            {
                $criteria->add(new TFilter('email', 'like', "%{$data->email}%"));
            }
            ...
            if ($objects)
            {
                ...
                // add a header row
                $tr->addRow();
                $tr->addCell('Autor', 'center', 'header', 3);
                ...
                // stores the file
                if (!file_exists("app/output/Autor.{$format}") OR is_writable("app/output/Autor.{$format}"))
                {
                    $tr->save("app/output/Autor.{$format}");
                }
                else
                {
                    throw new Exception(_t('Permission denied') . ': ' . "app/output/Autor.{$format}");
                }
                
                // open the report file
                parent::openFile("app/output/Autor.{$format}");
