...
class VendaFormList extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $cliente_id = new TDBUniqueSearch('cliente_id', 'db_livraria', 'Cliente', 'id', 'nome');


        // add the fields
        $this->form->addFields( [ new TLabel('Id') ], [ $id ], [ new TLabel('Data Venda') ], [ $data_venda ] );
        $this->form->addFields( [ new TLabel('Total') ], [ $total ], [ new TLabel('Cliente Id') ], [ $cliente_id ] );
        ...
        $column_cliente_id = new TDataGridColumn('{cliente->nome}', 'Cliente Id', 'left');
        ...
        //Formata em padrão moeda Brasil
        ...
        $action_group = new TDataGridActionGroup('Ação ', 'fa:th');
        ...
        try
        {
            ...
            if ($objects)
            {
                ...
                foreach ($objects as $object)
                {
                    ...
                    $object->data_venda = TDate::date2br($object->data_venda);
